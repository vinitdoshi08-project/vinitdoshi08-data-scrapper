...
def _send_brevo_email(to_email, subject, html_content, text_content, attachments=None):
    ...
    try:
        # We use a timeout of 10s for the HTTP request
        response = httpx.post(BREVO_API_URL, headers=headers, json=payload, timeout=10.0)
        
        if response.status_code in (201, 200, 202):
            print(f"[EmailService] Email sent successfully to {to_email}", flush=True)
            return True
        else:
...
def send_admin_notification(full_name, email, plan, registration_time, ip_address, os_type, country="Unknown"):
    ...
    # Send to admin
    return _send_brevo_email("vinit@aivhub.com", f"New Signup on Scrapify: {full_name}", html, text)
...
"""
    ...
    return _send_brevo_email(user_email, "Welcome to Scrapify!", html, text, attachments if attachments else None)
...
def send_otp_email(user_email, otp_code):
    ...
    return _send_brevo_email(user_email, f"Your Scrapify Verification Code: {otp_code}", html, text)
