...
export const openaiAdapter: AIAdapter = {
  ...
  async testKey(apiKey: string, modelId = 'gpt-4o-mini'): Promise<{ success: boolean; message: string }> {
    try {
      const res = await fetch('https://api.openai.com/v1/chat/completions', {
        method: 'POST',
        headers: {
      ...
      const err = await res.json().catch(() => ({}));
      return { success: false, message: err?.error?.message || 'Invalid OpenAI Key' };
    } catch (err: any) {
